...
pub(crate) fn num_to_bool(values: &[i32]) -> Vec<bool> {
    values.iter().map(|x| *x == 1).collect()
}

pub(crate) fn hamming_encode(bits: &[bool]) -> Vec<bool> {
...
pub(crate) fn hamming_decode(bits: &[bool]) -> (Vec<bool>, Option<usize>) {
...
pub(crate) fn parity_bits_count(len: usize) -> usize {
    let mut count = 0;
    let mut two_power = 1;
    let mut power = 0;
...
pub(crate) fn add_bits(bits: &[bool]) -> Vec<bool> {
...
pub(crate) fn calculate_parity_bits(bits: &[bool]) -> HashMap<usize, bool> {
